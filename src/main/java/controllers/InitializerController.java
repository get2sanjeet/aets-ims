...
public class InitializerController implements Initializable {
    ...
    public static void reloadData(List<ProductSpecification> productSpecifications) {
        if (productSpecifications.size() > 0 && productSpecifications.get(0) instanceof ProductSpecification) {
            List<ProductSpecification> productSpecificationsLatest = TransactionManager.getProductSpecifications();
            ProductSpecificationController.productSpecifications = productSpecificationsLatest;
            InventoryController.productSpecifications = productSpecificationsLatest;
        }
    }
    ...
    private void loadApplication() {
        ...
        try {
            ...
            base.setTitle("AETS Inventory System");
    ...
    class LoadRecords extends Task {
        ...
        protected Object call() throws Exception {
            ...
            ObservableList<ProductSpecification> productSpecificationObservableList = FXCollections.observableArrayList();
            ...
            final List<Customer> customers = TransactionManager.getCustomers();
            customersList.addAll(customers);
            customerID.addAll(customers.stream().map(Customer::getId).collect(Collectors.toList()));
            customerName.addAll(customers.stream().map(Customer::getName).collect(Collectors.toList()));
            ...
            OrdersController.customerIDName = customerName;

            // Getting values from customers result set
            final List<ProductSpecification> productSpecifications = TransactionManager.getProductSpecifications();
            productSpecificationObservableList.addAll(productSpecifications);
            InventoryController.productSpecifications = productSpecifications;
            ProductSpecificationController.productSpecifications = productSpecifications;

            //Setting Id and Name to SellsController, RentalsController, Accounts
            OrdersController.customerIDName = customerIDNameHolder;
            OrdersController.customerID = customerID;
            ...
            OrdersController.inventoryItem = itemIDNameForSale; //Setting item id and name for sale & RentalsController
            OrdersController.itemIDForSale = itemIDForSale;
            ...
            ObservableList<SalesOrderTableModel> sellsListByUser = FXCollections.observableArrayList();

            while(sellsList.next()) {
                sellsListByUser.add(new SalesOrderTableModel(sellsList.getString("purchaseID"),
            ...
            OrdersController.salesOrderTableModelList = sellsListByUser;
